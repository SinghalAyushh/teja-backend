...
module.exports = function (app, passport) {
    ...
    app.post('/signup', (req, res) => {
        ...
        User.findOne({ $or: [{ 'local.username': req.body.username }, { 'local.email': req.body.email }] }, function (err, user) {
            if (err) {
                return res.send(err);
            }
            if (user) {
    ...
    app.post('/verify', (req, res) => {
        User.findOne({ 'local.verifyusername': req.body.verifyusername }, function (err, result) {
            if (err) {
                return res.send(err);
            }
            if (!result) {
